...
def load_mycelium_mushrooms() -> set:
    """Parse Mycelium/Mycelium_config.md for mushroom node names like _/Name/."""
    names = set()
...
def consolidate_mycelium(apply_changes: bool = False) -> int:
    ...
    for stem, paths in stems.items():
        ...
        target = paths_sorted[0]
        sources = paths_sorted[1:]
        planned.append((target, sources))
        ...
        for target, sources in planned:
            for s in sources:
                ...
                try:
                    ...
                    print(f'Copied {s.relative_to(ROOT)} -> {dest.relative_to(ROOT)}')
                except Exception:
