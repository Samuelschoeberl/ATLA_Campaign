...
# Debug toggle: set PCS_DEBUG=1 in environment to enable extra debug prints
PCS_DEBUG: bool = bool(os.environ.get('PCS_DEBUG'))


# --- Lightweight file activity tracker (prints created/modified/renamed files) ---
class _FileTracker:
    def __init__(self):
    ...
    def note_created(self, p: Path) -> None:
        try:
            self.created.add(p.resolve())
        except Exception:
            self.created.add(p)

    def note_modified(self, p: Path) -> None:
        try:
            self.modified.add(p.resolve())
        except Exception:
            self.modified.add(p)

    def note_renamed(self, src: Path, dst: Path) -> None:
        try:
            self.renamed.append((src.resolve(), dst.resolve()))
        except Exception:
            self.renamed.append((src, dst))

    def note_deleted(self, p: Path) -> None:
        try:
            self.deleted.add(p.resolve())
        except Exception:
            self.deleted.add(p)
    ...
    def summary_lines(self) -> list[str]:
        out: list[str] = []
        if self.created:
            out.append('Files created:')
            for p in sorted(self.created):
                out.append(f'- {p}')
        if self.modified:
            out.append('Files modified:')
            for p in sorted(self.modified):
                out.append(f'- {p}')
        if self.renamed:
            out.append('Files renamed:')
            for s, d in self.renamed:
                out.append(f'- {s} -> {d}')
        if self.deleted:
            out.append('Files deleted:')
            for p in sorted(self.deleted):
                out.append(f'- {p}')
        return out
...
# Monkeypatch Path.write_text and Path.replace to track file writes/renames
_orig_write_text = Path.write_text
_orig_replace = Path.replace

# Save other originals we may patch for deletion/rename tracking
...
def _tracked_write_text(self: Path, data, encoding=None):
    prev_exists = self.exists()
    prev_text = None
    try:
        if prev_exists:
            try:
                prev_text = self.read_text(encoding='utf-8')
            except Exception:
                prev_text = None
    except Exception:
        prev_exists = False
    res = _orig_write_text(self, data, encoding=encoding if encoding is not None else 'utf-8')
    try:
        if not prev_exists:
            FILE_TRACKER.note_created(self)
        else:
            # consider it modified if content changed
            if prev_text is not None and prev_text != data:
                FILE_TRACKER.note_modified(self)
    except Exception:
        pass
    return res

def _tracked_replace(self: Path, target: Path, /):
    try:
        dst = _orig_replace(self, target)
    except Exception:
        # bubble
        raise
    try:
        FILE_TRACKER.note_renamed(self, Path(target))
    except Exception:
        pass
    return dst


def _tracked_unlink(self: Path, *args, **kwargs):
    # mark deletion then call original
    try:
        res = _orig_unlink(self, *args, **kwargs)
    except Exception:
        raise
    try:
        FILE_TRACKER.note_deleted(self)
    except Exception:
        pass
    return res


def _tracked_rename(self: Path, target, *args, **kwargs):
    try:
        res = _orig_rename(self, target, *args, **kwargs)
    except Exception:
        raise
    try:
        FILE_TRACKER.note_renamed(self, Path(target))
...
def _tracked_os_remove(path, *args, **kwargs):
    try:
        res = _orig_os_remove(path, *args, **kwargs)
    except Exception:
        raise
    try:
        FILE_TRACKER.note_deleted(Path(path))
    except Exception:
        pass
    return res


def _tracked_os_rename(src, dst, *args, **kwargs):
    try:
        res = _orig_os_rename(src, dst, *args, **kwargs)
    except Exception:
        raise
    try:
        FILE_TRACKER.note_renamed(Path(src), Path(dst))
    except Exception:
        pass
    return res


def _tracked_shutil_move(src, dst, *args, **kwargs):
    try:
        res = _orig_shutil_move(src, dst, *args, **kwargs)
    except Exception:
        raise
    try:
        FILE_TRACKER.note_renamed(Path(src), Path(dst))
...
def _apply_global_patches():
    """Apply global monkeypatches for write/replace/unlink/rename/remove/move."""
    # Avoid double-patching by checking identity
    try:
        if Path.write_text is _orig_write_text:
            Path.write_text = _tracked_write_text  # type: ignore
    except Exception:
        pass
    try:
        if Path.replace is _orig_replace:
            Path.replace = _tracked_replace  # type: ignore
    except Exception:
        pass
    try:
        if Path.unlink is _orig_unlink:
            Path.unlink = _tracked_unlink  # type: ignore
    except Exception:
        pass
    try:
        if Path.rename is _orig_rename:
            Path.rename = _tracked_rename  # type: ignore
    except Exception:
        pass
    try:
        if os.remove is _orig_os_remove:
            os.remove = _tracked_os_remove
    except Exception:
        pass
    try:
        if os.rename is _orig_os_rename:
            os.rename = _tracked_os_rename
    except Exception:
        pass
    try:
        if shutil.move is _orig_shutil_move:
            shutil.move = _tracked_shutil_move
    except Exception:
        pass


def _revert_global_patches():
    """Restore original functions patched by this module."""
    try:
        Path.write_text = _orig_write_text  # type: ignore
    except Exception:
        pass
    try:
        Path.replace = _orig_replace  # type: ignore
    except Exception:
        pass
    try:
...
@contextmanager
def tracked_file_io(scope_name: str | None = None):
    """Context manager that applies tracking monkeypatches while active
    and restores original functions on exit. Use this if you prefer scoped
    tracking instead of global monkeypatching.
    """
...
# apply monkeypatch globally by default to preserve existing behaviour
_apply_global_patches()
...
def find_character_file(pc: str | None, npc: str | None, file_arg: str | None) -> Path | None:
    ...
    if pc:
        candidates = []
        # Prefer a repository-level 'PC Character Sheets' folder if present
        try:
            script_dir = Path(__file__).resolve().parent
            pcs_root = None
            for p in script_dir.rglob('PC Character Sheets'):
                if p.is_dir():
                    pcs_root = p
                    break
            if pcs_root:
                # PCs/<PC>/Character Sheet.md
                candidates.append(pcs_root / pc / "Character Sheet.md")
                # PCs/<PC>/<PC> Character Sheet.md
                candidates.append(pcs_root / pc / f"{pc} Character Sheet.md")
        except Exception:
            pcs_root = None

        # Legacy locations: Players Part/PCs (respect configured pcs_root)
        cfg_pcs_root = get_config('pcs_root', 'Players Part/PCs')
        try:
            candidates.append(Path(cfg_pcs_root) / pc / "Character Sheet.md")
            candidates.append(Path(cfg_pcs_root) / pc / f"{pc} Character Sheet.md")
        except Exception:
        ...
        # Additional heuristic: search for plausible character sheets in adjacent
        # folders and nearby markdown files whose filename/stem contains the PC name
        try:
            root = Path('.')
            name_lower = pc.lower()
            ...
            for p in iter_md_files(root):
                stem = p.stem.lower()
                # plausible candidates: filename contains name and mentions 'character' or 'sheet',
                # or parent folder equals the pc name
                if name_lower in stem and ('character' in stem or 'sheet' in stem or p.parent.name.lower() == name_lower or p.parent.parent.name.lower() == name_lower):
                    return p.resolve()
        except Exception:
            pass

    # If --npc provided, try common NPC locations under configured DMs root
    if npc:
        candidates = []
        dms_cfg = get_config('dms_root', 'DMs Part')
        try:
            dms_base = Path(dms_cfg)
        except Exception:
            dms_base = Path('DMs Part')
        # DMs Part/NPCs/<NPC>/<NPC> Character Sheet.md
        candidates.append(dms_base.joinpath('NPCs') / npc / f"{npc} Character Sheet.md")
        # DMs Part/NPCs/<NPC> Character Sheet.md
        candidates.append(dms_base.joinpath('NPCs') / f"{npc} Character Sheet.md")
        # DMs Part/NPCs/<NPC>.md
        candidates.append(dms_base.joinpath('NPCs') / f"{npc}.md")
...
def parse_pcs_input_row(pc_name: str, pcs_path: Path | None = None) -> Dict[str, int]:
    ...
    for i in range(data_start, len(lines)):
        ...
        # skip separator rows that are only dashes like '| --- | --- |'
        if not cell_name:
            continue
        if re.fullmatch(r"-+", cell_name):
            continue
...
def insert_autogen_report(lines: list[str], inferred: Dict[str, int], overrides: Dict[str, int], unresolved: list[str]) -> list[str]:
    ...
    report_lines.append('')
    # inferred may be a dict or an ordered list of tuples. If dict, attempt
    # to order entries by PageRank score of a matching note file when
    # available. This makes the autogen report reflect importance of source
    # notes.
    inferred_list: list[tuple[str, int]] = []
    if isinstance(inferred, list):
        # attempt to coerce elements to (label, value)
        for item in inferred:
            if isinstance(item, (list, tuple)) and len(item) >= 2:
                inferred_list.append((str(item[0]), int(item[1])))
    elif isinstance(inferred, dict):
        # Build enriched tuples (label, value, pagerank) then reduce
        pr_map = {}
        try:
            pr_map = _load_pagerank_scores(Path('.'))
        except Exception:
            pr_map = {}
        enriched: list[tuple[str, int, float]] = []
        for k, v in inferred.items():
            score = 0.0
            try:
                note = _find_note_for_label(k)
                if note is not None:
                    score = float(pr_map.get(note.resolve(), 0.0))
            except Exception:
                score = 0.0
            enriched.append((str(k), int(v), float(score)))
        # sort by pagerank desc, then label
        enriched.sort(key=lambda t: (t[2], t[0]), reverse=True)
        # reduce to (label, value)
        inferred_list = [(k, v) for (k, v, _) in enriched]
    else:
        # unknown type: try to coerce
        try:
            for k, v in dict(inferred).items():
                inferred_list.append((k, int(v)))
        except Exception:
            inferred_list = []

    if inferred_list:
        report_lines.append('### Inferred values')
        for k, v in inferred_list:
...
def _find_note_for_label(label: str) -> Path | None:
    ...
    # Try to load PageRank scores for the workspace (if available). We map
    # pagerank keys (posix paths) to resolved Path objects so we can compare
    # candidate files directly.
    try:
        pagerank_scores: dict[Path, float] = _load_pagerank_scores(root)
    except Exception:
        pagerank_scores = {}

    best: Path | None = None
    # tuple of (pagerank_score, match_count) used for comparison
    ...
    for p in iter_md_files(root):
        ...
        if not stem_words:
            continue
        # match count heuristic as before
        match_count = 0
        for w in words:
        ...
        if match_count == 0:
            continue
        pr_score = float(pagerank_scores.get(p.resolve(), 0.0))

        # prefer higher pagerank, then higher match_count
        cand_key = (pr_score, match_count)
...
_PAGERANK_CACHE: dict[Path, float] | None = None


def _load_pagerank_scores(root: Path) -> dict[Path, float]:
    """Load pagerank scores (if available) and return a mapping from
    resolved Path -> float score. Uses Mycelium/pagerank.json if present.
    Results are cached for the process lifetime.
    """
    global _PAGERANK_CACHE
    if _PAGERANK_CACHE is None:
        _PAGERANK_CACHE = {}
        pr_path = Path(root) / 'Mycelium' / 'pagerank.json'
        ...
        if pr_path.exists():
            ...
            data = {}
        mapping: dict[Path, float] = {}
        for k, v in (data.items() if isinstance(data, dict) else []):
            try:
                # keys in pagerank.json are usually repository-relative posix paths
                cand = (Path(root) / k).resolve()
            except Exception:
                try:
                    cand = Path(k).resolve()
                except Exception:
                    continue
            try:
                mapping[cand] = float(v)
            except Exception:
                try:
                    mapping[cand] = float(str(v))
                except Exception:
                    mapping[cand] = 0.0
        _PAGERANK_CACHE = mapping
...
def main():
    ...
    p.add_argument('--formulas', help='Path to JSON formulas file', default=default_formulas)
    p.add_argument('--extend-formulas', action='store_true', help='Allow adding missing default formulas to the formulas file')
    default_pcs = get_config('pcs_input', 'pcs_input.md')
    p.add_argument('--input', help='Path to pcs_input.md (master PC table)', default=default_pcs)
    ...
    # allow overriding the pcs_input.md path via --input
    pcs_input_path = Path(getattr(args, 'input', 'pcs_input.md'))
    ...
    if getattr(args, 'levelup', None):
        ...
        exit_code = _levelup_pcs_input(args.levelup, args.name, getattr(args, 'totalRolledHP', None), pcs_path=pcs_input_path)
    ...
    if getattr(args, 'all', False):
        pcs_path = pcs_input_path
        if not pcs_path.exists():
            print(f'{pcs_path} not found; cannot run --all')
            sys.exit(2)
        txt = pcs_path.read_text(encoding='utf-8')
        lines = txt.splitlines()
        header_idx = None
    ...
    if getattr(args, 'sync', False):
        ...
        # collect PC names
        pc_names = names_from_input(pcs_input_path)
            ...
            if p.exists():
                ...
                def ensure_pc_sheet(name: str):
                    ...
                    if not candidate.exists():
                        ...
                        lines.append('')
                        lines.append('| Attribute         | Value | note                            | Auto |')
                        lines.append('| ----------------- | ----- | ------------------------------- | ---- |')
                        lines.append('| [[Max Hitpoints]] | 0     |                                 | Y    |')
                        lines.append('| [[HP]]            | 0     |                                 | N    |')
                        lines.append('| [[Evasion]]       | 0     |                                 | Y    |')
                        lines.append('| [[Armor]]         | 0     |                                 | Y    |')
                        lines.append('| [[Stress Level]]  | 0     | default is 0 at start of combat | N    |')
                        lines.append('')
                        lines.append('---')
                        lines.append('')
                        lines.append('## Secondary Stats')
                        lines.append('')
                        lines.append('| Stat                     | value | Notes                  | Auto |')
                        lines.append('| ------------------------ | ----- | ---------------------- | ---- |')
                        lines.append('| [[Attack Roll Modifier]] | 0     |                        | Y    |')
                        lines.append('| [[Waterbending DC]]      | 0     |                        | Y    |')
                        lines.append('| [[Earthbending DC]]      | 0     |                        | Y    |')
                        lines.append('| [[Firebending DC]]       | 0     |                        | Y    |')
                        lines.append('| [[Spiritbending DC]]     | 0     | (Avatar Spirit Bridge) | Y    |')
                        lines.append('')
                        lines.append('## [[Manually Rolled Hitpoints]]')
                        lines.append('| Level | Element | Rolled | Auto |')
                        lines.append('| ----- | ------- | ------ | ---- |')
                        lines.append('')
                        try:
        ...
        def ensure_pc_sheet(name: str):
            # Create under configured pcs_root (default Players Part/PCs)
            pcs_root = get_config('pcs_root', 'Players Part/PCs')
            try:
                folder = Path(pcs_root) / name
            except Exception:
                folder = Path('Players Part/PCs') / name
            folder.mkdir(parents=True, exist_ok=True)
            candidate = folder / "Character Sheet.md"
            alt = folder / f"{name} Character Sheet.md"
            if not candidate.exists() and not alt.exists():
                lines = []
                lines.append(f"**Name:** {name}")
                lines.append("")
        ...
        for nm in pc_names:
            ...
            try:
                ...
                try:
                    pcs_row = parse_pcs_input_row(nm, pcs_path=pcs_input_path)
                except Exception:
                    pcs_row = {}
    ...
    # Load formulas: prefer markdown table `secondary_stat_formula.md` if present
    ...
    if mdpath.exists():
        try:
            ...
            print('Failed to parse secondary_stat_formula.md; falling back to JSON')
            formulas = {}
    if not formulas:
    ...
    # Prefer primary stats from pcs_input.md when available. This ensures
    # the authoritative source for STR/DEX/CON/INT/WIS/CHA is pcs_input.md
    # rather than potentially stale or incomplete Character Sheet tables.
    try:
        pcs_row_for_stats = parse_pcs_input_row(args.pc, pcs_path=pcs_input_path)
    except Exception:
        pcs_row_for_stats = {}
    if pcs_row_for_stats:
        # Map common header names to the stat codes used by formulas
        mapping = {
            'str': 'STR', 'strength': 'STR', 'STR': 'STR',
            'dex': 'DEX', 'dexterity': 'DEX', 'DEX': 'DEX',
            'con': 'CON', 'constitution': 'CON', 'CON': 'CON',
            'int': 'INT', 'intelligence': 'INT', 'INT': 'INT',
            'wis': 'WIS', 'wisdom': 'WIS', 'WIS': 'WIS',
            'cha': 'CHA', 'charisma': 'CHA', 'CHA': 'CHA',
        }
        for k, v in list(pcs_row_for_stats.items()):
            if not k:
                continue
            key_norm = k.strip()
            # check direct mapping and lowercased variants
            mapped = None
            if key_norm in mapping:
                mapped = mapping[key_norm]
            else:
                lk = key_norm.lower()
                if lk in mapping:
                    mapped = mapping[lk]
            if mapped:
                ...
                stats[mapped] = n
        if PCS_DEBUG:
            print(f"[pcs-debug] Overrode core stats from pcs_input.md for {getattr(args, 'pc', None)}: { {k: stats[k] for k in ['STR','DEX','CON','INT','WIS','CHA'] } }")
    ...
    try:
        pcs_row = parse_pcs_input_row(args.pc, pcs_path=pcs_input_path)
    ...
    # Build a sorted autogen report using insert_autogen_report helper so
    # inferred values are ordered by pagerank when possible.
    report_lines = insert_autogen_report([], inferred, overrides, unresolved)
    # replace the top-level header to indicate printed-only
    if report_lines and report_lines[0].strip().lower().startswith('## autogen report'):
        report_lines[0] = '## Autogen Report (printed only)'
    ...
    if replacements:
        ...
        try:
            ...
            try:
                pcs_row_local = parse_pcs_input_row(args.pc, pcs_path=pcs_input_path)
            except Exception:
                pcs_row_local = {}
