# Fire/Firebending Moves/Level 1/Burning Fists.md
Action (0)
Level 1 (0)
Stress Level (0)
Damaging Move (0)

- Range: Melee (0)
- Fire Fire Attack Roll (0): Attacks Firebending Slot (2) times
- Damage: 1d6 + Strength (0) Bludgeoning + 1d12 fire

Firebending (0)
Firebending Move (0)
Player Root (0)

#player_root
#fire
#Level1


---

# Fire/Firebending Moves/Level 1/Control Flame.md
Player Root/Rules/Bending Rules/Spirit/Spiritbending Moves/Utility/Utility (0)
Level 1 (0)

- Range: Player Root/Rules/Bending Rules/Fire/Mechanics/Firebending DC (0) meter
- You manipulate a flame within range

Firebending (0)
!Player Root/Rules/Bending Rules/Fire/Mechanics/Firebending DC (0)
Firebending Move (0)
Player Root (0)

#player_root
#fire
#Level1


---

# Fire/Firebending Moves/Level 1/Flame Jet.md
Action (0)
Level 1 (0)

- **Range:** 5 \* Firebending Slot (2) meters.
- **Damage:** Firebending Slot (2) \* 1 d10 fire
- Fire Fire Attack Roll (0)
- Unleash a burst of flames in a straight line, damaging one target

Firebending (0)
Damaging Move (0)
Firebending Move (0)
Player Root (0)

#player_root
#fire
#Level1


---

# Fire/Firebending Moves/Level 1/Smoldering Focus.md
Smoldering Focus

Utility / Support (0)
Level (0): 1
Additional attributes (0): Stress-aware

- Range: Self (0)
- **Duration:** Instant
- Description: Calm your breath and focus heat into a small, steady kernel for control or precision.

**Effects:**

- Roll: Roll +firebending.
- On a 10+: Reduce your Stress by 1. Choose one: +1 forward to your next firebending roll, or suppress a small fire for 1 minute.
- On a 7–9: No stress change; gain +1 forward to your next firebending roll.
- On a miss (6 or less): Gain +1 Stress and the GM introduces a small complication.

Fire (1)
Firebending Move (0)
Player Root (0)

#player_root
#fire
#Level1


---

# Water/Waterbending Moves/Level 1/Healing Water.md
Cyclic Flow (0)
Level 1 (0)

- **Cost:** 1-3 Water Charges.
- **Type:** Action (0).
- Duration: 1 round Concentration.
- **Range:** Touch (through water).
- **Effect:**  
    You channel healing energy through water, restoring health to a creature. 
	- If the creature is unconscious the creature becomes stable.
    - **Healing:** 2d8 per charge + Int (8) modifier at the beginning of your next Turn.
    - **Limitations:** Cannot heal constructs or undead.


!Cyclic Flow (0)
Waterbending (0)
Level 1 (0)
Player Root (0)

#player_root
#water
#Level1


---

# Water/Waterbending Moves/Level 1/Piercing Water.md
Action (0)
Level 1 (0)

    The waterbender shoots a sharp, pressurized stream of water at a target.
    - Water Water Attack Rolll (0)

- Int (8)

Tags:
#Waterbending
Waterbending (0)
Level 1 (0)
Player Root (0)

#player_root
#Level1 #water


---

# Water/Waterbending Moves/Level 1/Puffy Clouds.md
**Type:** Waterbending spiritbending sub-skill  

**Effect:**  
The bender draws a small amount of water and vaporizes it into a dense mist that spreads over a small to medium area. All creatures within the mist gain Fire Armour.  

**Mechanics (TTRPG style):**

- _Cost:_ 1-5 Water Charges (0)
    
- _Area:_ 1* Water Charges (0) Meter radius 
    
- Concentration: Water Charges (0) Turns. Every Creature in the area has 10 * Water Charges (0) Fire Armor definition (0).
    
- _Effect:_ Gradual healing, mental clarity, emotional soothing
    
- _Interactions:_
    
    - Can be _moved_ by airbenders using wind techniques.
        
    - Can be _dispersed_ or _destroyed_ by strong firebending or extreme heat.
        
        

**Advanced Technique (Mastery Level):**

- _Cleansing Veil:_ Condenses the mist into a swirling cloak around allies, granting them resistance to poison, disease, or negative spiritual effects for a short duration.
Waterbending (0)
Level 1 (0)
Player Root (0)

#player_root
#water
#Level1


---

# Water/Waterbending Moves/Level 1/Water Whip.md
#### **Water Whip (Level 1 Ability)**

- **Cost:** 1 Water Charge (24).
- **Range:** 20 ft.
- **Action (0):** 1 Action (0).
- **Effect:**  
   The waterbender lashes out with a tendril of water, striking a target within range.
  - **Water Water Attack Rolll (0):** Intelligence + Proficiency (0) Bonus.
  - **Damage:** 1d8 + Int|Int (0) modifier bludgeoning damage.
  - **Special:** The target must succeed on a Strength saving throw (DC 10 + Int|Int (0) modifier + Proficiency (0)) or be knocked back 10 feet.

**Links**:

- Strength (0)
- _Rules (0)
- Water Whip (0)
- Proficiency (0)
- Int (8)

Tags:
#water
#player_root
#Level1


---

# Water/Waterbending Moves/Level 2/Freeze.md
Reaction (0)

- **Range:** 5 * Waterbending Level (0)meter.
- **Effect:**  
    The waterbender can attempt to freeze a creature that has been hit by a Move using Water Charges (0). It has to succeed a Strength Saving throw against your Player Root/Rules/Bending Rules/Water/Mechanics/Waterbending DC (0) or become Immobilised (0).


**Links**:
- !Waterbending Level (0)
Waterbending (0)
Level 2 (0)
Player Root (0)

#player_root
#water
#Level2


---

# Water/Waterbending Moves/Level 2/Ice Daggers.md
#### **Ice Daggers (2nd Level Move)**

- **Cost:** 1-2*CL (3) Water Charges (0).
- **Type:** Action (0).
- **Range:** 20 ft.
- **Effect:**  
    The waterbender creates sharp daggers of ice to hurl at targets.
- Water Attack Rolll (0)
- **Damage:** 1d8 + Int (8) modifier piercing damage per dagger (can create up to 2*CL (3) daggers).

**Links**:
- CL (3)
- Ice Daggers (0)
- Int (8)

Tags:
#water 
#player_root
#Level2


---

# Water/Waterbending Moves/Level 2/Wall of water.md
#### **Wall of Water (2nd Level Move)**

- **Cost:** 3 Water Charges.
- **Type:** Action (0).
- **Range:** 60 ft.
- **Effect:**  
    The waterbender creates a wall of water up to 30 ft long, 10 ft high, and 1 ft thick.
    - **Duration:** Concentration, up to 1 minute.
    - **Effect:** Provides partial cover and can extinguish flames.

**Links**:
- Wall of water (0)


Tags:
#Waterbendingx 
#water 
#player_root
#Level2


---

# Water/Waterbending Moves/Level 2/Water Tentacles.md
Bonus Action (0)
Level 3 (0)
Cyclic Flow (0)

- Range: Self (0).
- **Duration:** 1 Turn.
- Description The waterbender summons two water tentacles to attack or grapple targets.

**Effects:**
    - Water Attack Rolll (0)
    - **Damage:** 1d10 bludgeoning damage per tentacle.
    - **Grapple:** A creature hit by the tentacle must succeed on a Strength saving throw against your Player Root/Rules/Bending Rules/Water/Mechanics/Waterbending DC (0) or be restrained.

Element (0)
#### **Water Tentacles (2nd Level Move)**

- **Cost:** 2 Water Charges.
- **Type:** Action (0).
- **Range:** 15 ft.
- **Effect:**  
    The waterbender summons two water tentacles to attack or grapple targets.
    - **Water Attack Rolll (0):** Int (8) + Proficiency (0).
    - **Damage:** 1d10 bludgeoning damage per tentacle.
    - **Grapple:** A creature hit by the tentacle must succeed on a Strength saving throw or be restrained.


Tags:
#player_root
#Level2
#water 


---

# Water/Waterbending Moves/Utility/Mistbending.md
#Action 
- **Creating Mist:**
- **Cost:** 1 Water Charge per 3 meter radius .
- **Range:** 3 meter * Water Charges (0) radius centered on self.
    - Will grow 10 feet per round until full sized
    - Creates a heavily obscured area for concealment that can be absorbed to regain charges (10 meter radius per Turn) or blown await by air.


#water 
#level1
#player_root



---

# Water/Waterbending Moves/Utility/Shape Water.md
#### **Shape Water (Cantrip)**
#Action 
#Concentration 

- **Cost:** None.
- **Range:** water (2) * 3 meter

- **Duration:** At Will. Concentration.
- **Effect:**  
    You can manipulate a water, shaping it into simple forms, moving it, or changing its state (solid, liquid, mist) temporarily.
    - **Limitations:** Cannot deal damage but can be used for utility (e.g., extinguishing flames, creating distractions). The higher your Level is the more water can be shaped. Might need an Int ability check.


Tags:
#Waterbending
#water 
#Level1 
#player_root


---

