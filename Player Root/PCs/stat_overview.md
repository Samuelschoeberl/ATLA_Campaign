# Stat Overview

## Global environmental variables


| Name                           | Value | Tags                                                         | File                                                 |
| [[environmental_water_charge]] | 4/4 | #variable #secondary_stat #template #environmental_variables | Player Root/variable/environmental/environmental_water_charge.md |


## Per-PC extracted stats

_(Only shows variables tagged with #vitality, #defensive, or #bending_slot, or matching patterns)_


### Anju


**Vitality**


| Key           | Value | Source File                            |
| current_hp | 0    | Player Root/variable/PC_variables/Anju/Anju_current_hp.md |
| max_hp | 38    | Player Root/variable/PC_variables/Anju/Anju_max_hp.md |
| max_hp | 38    | Player Root/PCs/Anju/Anju Character Sheet.md |
| current_hp | 0    | Player Root/PCs/Anju/Anju Character Sheet.md |
| ready | no    | Player Root/PCs/Anju/Anju Character Sheet.md |


**Defensive**


| Key           | Value | Source File                            |
| Barrier | 0    | Player Root/variable/PC_variables/Anju/Anju_Barrier.md |
| Evasion | 11    | Player Root/variable/PC_variables/Anju/Anju_Evasion.md |
| Fire Armor | 2    | Player Root/variable/PC_variables/Anju/Anju_Fire Armor.md |
| General Armor | 1    | Player Root/variable/PC_variables/Anju/Anju_General Armor.md |
| Ice Armor | 0    | Player Root/variable/PC_variables/Anju/Anju_Ice Armor.md |
| Physical Armor | 0    | Player Root/variable/PC_variables/Anju/Anju_Physical Armor.md |
| Spirit Armor | 0    | Player Root/variable/PC_variables/Anju/Anju_Spirit Armor.md |
| general_armor | 1    | Player Root/variable/PC_variables/Anju/Anju_general_armor.md |


**Consumable Resources**


| Resource Name            | Current/Max | Source File                                       |
| ------------------------ | ----------- | ------------------------------------------------- |
| Earthbending slot        | 1/3         | Player Root/PCs/Anju/Anju Character Sheet.md      |
| Waterbottle charge       | 2/4         | Player Root/PCs/Anju/Anju Character Sheet.md      |


### Ash


**Vitality**


| Key           | Value | Source File                            |
| current_hp | 26    | Player Root/variable/PC_variables/Ash/Ash_current_hp.md |
| max_hp | 26    | Player Root/variable/PC_variables/Ash/Ash_max_hp.md |
| max_hp | 26    | Player Root/PCs/Ash/Ash Character Sheet.md |
| current_hp | 26    | Player Root/PCs/Ash/Ash Character Sheet.md |
| ready | no    | Player Root/PCs/Ash/Ash Character Sheet.md |


**Defensive**


| Key           | Value | Source File                            |
| Barrier | 0    | Player Root/variable/PC_variables/Ash/Ash_Barrier.md |
| Evasion | 11    | Player Root/variable/PC_variables/Ash/Ash_Evasion.md |
| Fire Armor | 0    | Player Root/variable/PC_variables/Ash/Ash_Fire Armor.md |
| General Armor | 0    | Player Root/variable/PC_variables/Ash/Ash_General Armor.md |
| Ice Armor | 2    | Player Root/variable/PC_variables/Ash/Ash_Ice Armor.md |
| Physical Armor | 0    | Player Root/variable/PC_variables/Ash/Ash_Physical Armor.md |
| Spirit Armor | 1    | Player Root/variable/PC_variables/Ash/Ash_Spirit Armor.md |
| general_armor | 0    | Player Root/variable/PC_variables/Ash/Ash_general_armor.md |


**Consumable Resources**


| Resource Name            | Current/Max | Source File                                       |
| ------------------------ | ----------- | ------------------------------------------------- |
| Firebending slot         | 6/6         | Player Root/PCs/Ash/Ash Character Sheet.md        |
| Spiritbending slot       | 1/1         | Player Root/PCs/Ash/Ash Character Sheet.md        |


### Blimp


**Vitality**


| Key           | Value | Source File                            |
| current_hp | 54    | Player Root/variable/PC_variables/Blimp/Blimp_current_hp.md |
| max_hp | 54    | Player Root/variable/PC_variables/Blimp/Blimp_max_hp.md |
| max_hp | 54    | Player Root/PCs/Blimp/Blimp character sheet.md |
| current_hp | 54    | Player Root/PCs/Blimp/Blimp character sheet.md |
| ready | no    | Player Root/PCs/Blimp/Blimp character sheet.md |


**Defensive**


| Key           | Value | Source File                            |
| Barrier | 0    | Player Root/variable/PC_variables/Blimp/Blimp_Barrier.md |
| Evasion | 12    | Player Root/variable/PC_variables/Blimp/Blimp_Evasion.md |
| Fire Armor | 0    | Player Root/variable/PC_variables/Blimp/Blimp_Fire Armor.md |
| General Armor | 2    | Player Root/variable/PC_variables/Blimp/Blimp_General Armor.md |
| Ice Armor | 0    | Player Root/variable/PC_variables/Blimp/Blimp_Ice Armor.md |
| Physical Armor | 0    | Player Root/variable/PC_variables/Blimp/Blimp_Physical Armor.md |
| Spirit Armor | 0    | Player Root/variable/PC_variables/Blimp/Blimp_Spirit Armor.md |
| general_armor | 2    | Player Root/variable/PC_variables/Blimp/Blimp_general_armor.md |


**Consumable Resources**


| Resource Name            | Current/Max | Source File                                       |
| ------------------------ | ----------- | ------------------------------------------------- |
| Earthbending slot        | 6/6         | Player Root/PCs/Blimp/Blimp character sheet.md    |


### DMs_Air


**Vitality**


| Key           | Value | Source File                            |
| current_hp | 120    | Player Root/variable/PC_variables/DMs_Air/DMs_Air_current_hp.md |
| max_hp | 120    | Player Root/variable/PC_variables/DMs_Air/DMs_Air_max_hp.md |
| max_hp | 120    | Player Root/PCs/DMs_Air/DMs_Air character sheet.md |
| current_hp | 120    | Player Root/PCs/DMs_Air/DMs_Air character sheet.md |


**Defensive**


| Key           | Value | Source File                            |
| Barrier | 0    | Player Root/variable/PC_variables/DMs_Air/DMs_Air_Barrier.md |
| Evasion | 26    | Player Root/variable/PC_variables/DMs_Air/DMs_Air_Evasion.md |
| Fire Armor | 0    | Player Root/variable/PC_variables/DMs_Air/DMs_Air_Fire Armor.md |
| General Armor | 0    | Player Root/variable/PC_variables/DMs_Air/DMs_Air_General Armor.md |
| Ice Armor | 0    | Player Root/variable/PC_variables/DMs_Air/DMs_Air_Ice Armor.md |
| Physical Armor | 0    | Player Root/variable/PC_variables/DMs_Air/DMs_Air_Physical Armor.md |
| Spirit Armor | 0    | Player Root/variable/PC_variables/DMs_Air/DMs_Air_Spirit Armor.md |


**Consumable Resources**


| Resource Name            | Current/Max | Source File                                       |
| ------------------------ | ----------- | ------------------------------------------------- |
| Airbending slot          | 30/30       | Player Root/PCs/DMs_Air/DMs_Air character sheet.m |
| Danger Sense Reactions   | 10/10       | Player Root/PCs/DMs_Air/DMs_Air character sheet.m |


### DMs_Earth


**Vitality**


| Key           | Value | Source File                            |
| current_hp | 216    | Player Root/variable/PC_variables/DMs_Earth/DMs_Earth_current_hp.md |
| max_hp | 216    | Player Root/variable/PC_variables/DMs_Earth/DMs_Earth_max_hp.md |
| max_hp | 216    | Player Root/PCs/DMs_Earth/DMs_Earth character sheet.md |
| current_hp | 216    | Player Root/PCs/DMs_Earth/DMs_Earth character sheet.md |


**Defensive**


| Key           | Value | Source File                            |
| Barrier | 0    | Player Root/variable/PC_variables/DMs_Earth/DMs_Earth_Barrier.md |
| Evasion | 12    | Player Root/variable/PC_variables/DMs_Earth/DMs_Earth_Evasion.md |
| Fire Armor | 0    | Player Root/variable/PC_variables/DMs_Earth/DMs_Earth_Fire Armor.md |
| General Armor | 10    | Player Root/variable/PC_variables/DMs_Earth/DMs_Earth_General Armor.md |
| Ice Armor | 0    | Player Root/variable/PC_variables/DMs_Earth/DMs_Earth_Ice Armor.md |
| Physical Armor | 0    | Player Root/variable/PC_variables/DMs_Earth/DMs_Earth_Physical Armor.md |
| Spirit Armor | 0    | Player Root/variable/PC_variables/DMs_Earth/DMs_Earth_Spirit Armor.md |


**Consumable Resources**


| Resource Name            | Current/Max | Source File                                       |
| ------------------------ | ----------- | ------------------------------------------------- |
| Earthbending slot        | 30/30       | Player Root/PCs/DMs_Earth/DMs_Earth character she |


### DMs_Fire


**Vitality**


| Key           | Value | Source File                            |
| current_hp | 186    | Player Root/variable/PC_variables/DMs_Fire/DMs_Fire_current_hp.md |
| max_hp | 186    | Player Root/variable/PC_variables/DMs_Fire/DMs_Fire_max_hp.md |
| max_hp | 186    | Player Root/PCs/DMs_Fire/DMs_Fire character sheet.md |
| current_hp | 186    | Player Root/PCs/DMs_Fire/DMs_Fire character sheet.md |


**Defensive**


| Key           | Value | Source File                            |
| Barrier | 0    | Player Root/variable/PC_variables/DMs_Fire/DMs_Fire_Barrier.md |
| Evasion | 12    | Player Root/variable/PC_variables/DMs_Fire/DMs_Fire_Evasion.md |
| Fire Armor | 0    | Player Root/variable/PC_variables/DMs_Fire/DMs_Fire_Fire Armor.md |
| General Armor | 0    | Player Root/variable/PC_variables/DMs_Fire/DMs_Fire_General Armor.md |
| Ice Armor | 10    | Player Root/variable/PC_variables/DMs_Fire/DMs_Fire_Ice Armor.md |
| Physical Armor | 0    | Player Root/variable/PC_variables/DMs_Fire/DMs_Fire_Physical Armor.md |
| Spirit Armor | 0    | Player Root/variable/PC_variables/DMs_Fire/DMs_Fire_Spirit Armor.md |


**Consumable Resources**


| Resource Name            | Current/Max | Source File                                       |
| ------------------------ | ----------- | ------------------------------------------------- |
| Firebending slot         | 30/30       | Player Root/PCs/DMs_Fire/DMs_Fire character sheet |


### DMs_Spirit


**Vitality**


| Key           | Value | Source File                            |
| current_hp | 152    | Player Root/variable/PC_variables/DMs_Spirit/DMs_Spirit_current_hp.md |
| max_hp | 152    | Player Root/variable/PC_variables/DMs_Spirit/DMs_Spirit_max_hp.md |
| max_hp | 152    | Player Root/PCs/DMs_Spirit/DMs_Spirit character sheet.md |
| current_hp | 152    | Player Root/PCs/DMs_Spirit/DMs_Spirit character sheet.md |


**Defensive**


| Key           | Value | Source File                            |
| Barrier | 0    | Player Root/variable/PC_variables/DMs_Spirit/DMs_Spirit_Barrier.md |
| Evasion | 13    | Player Root/variable/PC_variables/DMs_Spirit/DMs_Spirit_Evasion.md |
| Fire Armor | 0    | Player Root/variable/PC_variables/DMs_Spirit/DMs_Spirit_Fire Armor.md |
| General Armor | 0    | Player Root/variable/PC_variables/DMs_Spirit/DMs_Spirit_General Armor.md |
| Ice Armor | 0    | Player Root/variable/PC_variables/DMs_Spirit/DMs_Spirit_Ice Armor.md |
| Physical Armor | 0    | Player Root/variable/PC_variables/DMs_Spirit/DMs_Spirit_Physical Armor.md |
| Spirit Armor | 10    | Player Root/variable/PC_variables/DMs_Spirit/DMs_Spirit_Spirit Armor.md |


**Consumable Resources**


| Resource Name            | Current/Max | Source File                                       |
| ------------------------ | ----------- | ------------------------------------------------- |
| Spiritbending slot       | 10/10       | Player Root/PCs/DMs_Spirit/DMs_Spirit character s |


### DMs_Water


**Vitality**


| Key           | Value | Source File                            |
| current_hp | 152    | Player Root/variable/PC_variables/DMs_Water/DMs_Water_current_hp.md |
| max_hp | 152    | Player Root/variable/PC_variables/DMs_Water/DMs_Water_max_hp.md |
| max_hp | 152    | Player Root/PCs/DMs_Water/DMs_Water character sheet.md |
| current_hp | 152    | Player Root/PCs/DMs_Water/DMs_Water character sheet.md |


**Defensive**


| Key           | Value | Source File                            |
| Barrier | 0    | Player Root/variable/PC_variables/DMs_Water/DMs_Water_Barrier.md |
| Evasion | 14    | Player Root/variable/PC_variables/DMs_Water/DMs_Water_Evasion.md |
| Fire Armor | 10    | Player Root/variable/PC_variables/DMs_Water/DMs_Water_Fire Armor.md |
| General Armor | 0    | Player Root/variable/PC_variables/DMs_Water/DMs_Water_General Armor.md |
| Ice Armor | 0    | Player Root/variable/PC_variables/DMs_Water/DMs_Water_Ice Armor.md |
| Physical Armor | 0    | Player Root/variable/PC_variables/DMs_Water/DMs_Water_Physical Armor.md |
| Spirit Armor | 0    | Player Root/variable/PC_variables/DMs_Water/DMs_Water_Spirit Armor.md |


**Consumable Resources**


| Resource Name            | Current/Max | Source File                                       |
| ------------------------ | ----------- | ------------------------------------------------- |
| Waterbottle charge       | 20          | Player Root/PCs/DMs_Water/DMs_Water character she |


### Kana


**Vitality**


| Key           | Value | Source File                            |
| current_hp | 24    | Player Root/variable/PC_variables/Kana/Kana_current_hp.md |
| max_hp | 24    | Player Root/variable/PC_variables/Kana/Kana_max_hp.md |
| max_hp | 24    | Player Root/PCs/Kana/Kana character sheet.md |
| current_hp | 24    | Player Root/PCs/Kana/Kana character sheet.md |


**Defensive**


| Key           | Value | Source File                            |
| Barrier | 0    | Player Root/variable/PC_variables/Kana/Kana_Barrier.md |
| Evasion | 19    | Player Root/variable/PC_variables/Kana/Kana_Evasion.md |
| Fire Armor | 0    | Player Root/variable/PC_variables/Kana/Kana_Fire Armor.md |
| General Armor | 0    | Player Root/variable/PC_variables/Kana/Kana_General Armor.md |
| Ice Armor | 0    | Player Root/variable/PC_variables/Kana/Kana_Ice Armor.md |
| Physical Armor | 0    | Player Root/variable/PC_variables/Kana/Kana_Physical Armor.md |
| Spirit Armor | 1    | Player Root/variable/PC_variables/Kana/Kana_Spirit Armor.md |


**Consumable Resources**


| Resource Name            | Current/Max | Source File                                       |
| ------------------------ | ----------- | ------------------------------------------------- |
| Airbending slot          | 3/3         | Player Root/PCs/Kana/Kana character sheet.md      |
| Danger Sense Reactions   | 1/1         | Player Root/PCs/Kana/Kana character sheet.md      |
| Spiritbending slot       | 1/1         | Player Root/PCs/Kana/Kana character sheet.md      |


### Leif


**Vitality**


| Key           | Value | Source File                            |
| current_hp | 30    | Player Root/variable/PC_variables/Leif/Leif_current_hp.md |
| max_hp | 30    | Player Root/variable/PC_variables/Leif/Leif_max_hp.md |
| max_hp | 30    | Player Root/PCs/Leif/Leif character sheet.md |
| current_hp | 30    | Player Root/PCs/Leif/Leif character sheet.md |


**Defensive**


| Key           | Value | Source File                            |
| Barrier | 0    | Player Root/variable/PC_variables/Leif/Leif_Barrier.md |
| Evasion | 19    | Player Root/variable/PC_variables/Leif/Leif_Evasion.md |
| Fire Armor | 1    | Player Root/variable/PC_variables/Leif/Leif_Fire Armor.md |
| General Armor | 0    | Player Root/variable/PC_variables/Leif/Leif_General Armor.md |
| Ice Armor | 0    | Player Root/variable/PC_variables/Leif/Leif_Ice Armor.md |
| Physical Armor | 0    | Player Root/variable/PC_variables/Leif/Leif_Physical Armor.md |
| Spirit Armor | 0    | Player Root/variable/PC_variables/Leif/Leif_Spirit Armor.md |


**Consumable Resources**


| Resource Name            | Current/Max | Source File                                       |
| ------------------------ | ----------- | ------------------------------------------------- |
| Airbending slot          | 3/3         | Player Root/PCs/Leif/Leif character sheet.md      |
| Danger Sense Reactions   | 1/1         | Player Root/PCs/Leif/Leif character sheet.md      |
| Waterbottle charge       | 2           | Player Root/PCs/Leif/Leif character sheet.md      |


### Mahogany


**Vitality**


| Key           | Value | Source File                            |
| current_hp | 26    | Player Root/variable/PC_variables/Mahogany/Mahogany_current_hp.md |
| max_hp | 26    | Player Root/variable/PC_variables/Mahogany/Mahogany_max_hp.md |
| max_hp | 26    | Player Root/PCs/Mahogany/Mahogany character sheet.md |
| current_hp | 26    | Player Root/PCs/Mahogany/Mahogany character sheet.md |
| ready | no    | Player Root/PCs/Mahogany/Mahogany character sheet.md |


**Defensive**


| Key           | Value | Source File                            |
| Barrier | 0    | Player Root/variable/PC_variables/Mahogany/Mahogany_Barrier.md |
| Evasion | 15    | Player Root/variable/PC_variables/Mahogany/Mahogany_Evasion.md |
| Fire Armor | 0    | Player Root/variable/PC_variables/Mahogany/Mahogany_Fire Armor.md |
| General Armor | 0    | Player Root/variable/PC_variables/Mahogany/Mahogany_General Armor.md |
| Ice Armor | 0    | Player Root/variable/PC_variables/Mahogany/Mahogany_Ice Armor.md |
| Physical Armor | 0    | Player Root/variable/PC_variables/Mahogany/Mahogany_Physical Armor.md |
| Spirit Armor | 2    | Player Root/variable/PC_variables/Mahogany/Mahogany_Spirit Armor.md |
| general_armor | 0    | Player Root/variable/PC_variables/Mahogany/Mahogany_general_armor.md |


**Consumable Resources**


| Resource Name            | Current/Max | Source File                                       |
| ------------------------ | ----------- | ------------------------------------------------- |
| Airbending slot          | 3/3         | Player Root/PCs/Mahogany/Mahogany character sheet |
| Danger Sense Reactions   | 1/1         | Player Root/PCs/Mahogany/Mahogany character sheet |
| Spiritbending slot       | 2/2         | Player Root/PCs/Mahogany/Mahogany character sheet |


### Nafas


**Vitality**


| Key           | Value | Source File                            |
| current_hp | 32    | Player Root/variable/PC_variables/Nafas/Nafas_current_hp.md |
| max_hp | 32    | Player Root/variable/PC_variables/Nafas/Nafas_max_hp.md |
| max_hp | 32    | Player Root/PCs/Nafas/Nafas character sheet.md |
| current_hp | 32    | Player Root/PCs/Nafas/Nafas character sheet.md |


**Defensive**


| Key           | Value | Source File                            |
| Barrier | 0    | Player Root/variable/PC_variables/Nafas/Nafas_Barrier.md |
| Evasion | 17    | Player Root/variable/PC_variables/Nafas/Nafas_Evasion.md |
| Fire Armor | 0    | Player Root/variable/PC_variables/Nafas/Nafas_Fire Armor.md |
| General Armor | 0    | Player Root/variable/PC_variables/Nafas/Nafas_General Armor.md |
| Ice Armor | 0    | Player Root/variable/PC_variables/Nafas/Nafas_Ice Armor.md |
| Physical Armor | 0    | Player Root/variable/PC_variables/Nafas/Nafas_Physical Armor.md |
| Spirit Armor | 2    | Player Root/variable/PC_variables/Nafas/Nafas_Spirit Armor.md |


**Consumable Resources**


| Resource Name            | Current/Max | Source File                                       |
| ------------------------ | ----------- | ------------------------------------------------- |
| Spiritbending slot       | 2/2         | Player Root/PCs/Nafas/Nafas character sheet.md    |


### Plulp


**Vitality**


| Key           | Value | Source File                            |
| current_hp | 188    | Player Root/variable/PC_variables/Plulp/Plulp_current_hp.md |
| max_hp | 188    | Player Root/variable/PC_variables/Plulp/Plulp_max_hp.md |
| max_hp | 188    | Player Root/PCs/Plulp/Plulp character sheet.md |
| current_hp | 188    | Player Root/PCs/Plulp/Plulp character sheet.md |
| ready | no    | Player Root/PCs/Plulp/Plulp character sheet.md |


**Defensive**


| Key           | Value | Source File                            |
| Barrier | 0    | Player Root/variable/PC_variables/Plulp/Plulp_Barrier.md |
| Evasion | 11    | Player Root/variable/PC_variables/Plulp/Plulp_Evasion.md |
| Fire Armor | 0    | Player Root/variable/PC_variables/Plulp/Plulp_Fire Armor.md |
| General Armor | 6    | Player Root/variable/PC_variables/Plulp/Plulp_General Armor.md |
| Ice Armor | 0    | Player Root/variable/PC_variables/Plulp/Plulp_Ice Armor.md |
| Physical Armor | 0    | Player Root/variable/PC_variables/Plulp/Plulp_Physical Armor.md |
| Spirit Armor | 3    | Player Root/variable/PC_variables/Plulp/Plulp_Spirit Armor.md |
| general_armor | 6    | Player Root/variable/PC_variables/Plulp/Plulp_general_armor.md |


**Consumable Resources**


| Resource Name            | Current/Max | Source File                                       |
| ------------------------ | ----------- | ------------------------------------------------- |
| Earthbending slot        | 7/18        | Player Root/PCs/Plulp/Plulp character sheet.md    |
| Spiritbending slot       | 3/3         | Player Root/PCs/Plulp/Plulp character sheet.md    |


### Puy


**Vitality**


| Key           | Value | Source File                            |
| current_hp | 48    | Player Root/variable/PC_variables/Puy/Puy_current_hp.md |
| max_hp | 48    | Player Root/variable/PC_variables/Puy/Puy_max_hp.md |
| max_hp | 48    | Player Root/PCs/Puy/Puy Character Sheet.md |
| current_hp | 48    | Player Root/PCs/Puy/Puy Character Sheet.md |
| ready | no    | Player Root/PCs/Puy/Puy Character Sheet.md |


**Defensive**


| Key           | Value | Source File                            |
| Barrier | 0    | Player Root/variable/PC_variables/Puy/Puy_Barrier.md |
| Evasion | 13    | Player Root/variable/PC_variables/Puy/Puy_Evasion.md |
| Fire Armor | 0    | Player Root/variable/PC_variables/Puy/Puy_Fire Armor.md |
| General Armor | 2    | Player Root/variable/PC_variables/Puy/Puy_General Armor.md |
| Ice Armor | 1    | Player Root/variable/PC_variables/Puy/Puy_Ice Armor.md |
| Physical Armor | 0    | Player Root/variable/PC_variables/Puy/Puy_Physical Armor.md |
| Spirit Armor | 0    | Player Root/variable/PC_variables/Puy/Puy_Spirit Armor.md |
| general_armor | 2    | Player Root/variable/PC_variables/Puy/Puy_general_armor.md |


**Consumable Resources**


| Resource Name            | Current/Max | Source File                                       |
| ------------------------ | ----------- | ------------------------------------------------- |
| Firebending slot         | 3/3         | Player Root/PCs/Puy/Puy Character Sheet.md        |
| Earthbending slot        | 4/6         | Player Root/PCs/Puy/Puy Character Sheet.md        |


### Rio


**Vitality**


| Key           | Value | Source File                            |
| current_hp | 36    | Player Root/variable/PC_variables/Rio/Rio_current_hp.md |
| max_hp | 36    | Player Root/variable/PC_variables/Rio/Rio_max_hp.md |
| max_hp | 36    | Player Root/PCs/Rio/Rio Character Sheet.md |
| current_hp | 36    | Player Root/PCs/Rio/Rio Character Sheet.md |
| ready | no    | Player Root/PCs/Rio/Rio Character Sheet.md |


**Defensive**


| Key           | Value | Source File                            |
| Barrier | 0    | Player Root/variable/PC_variables/Rio/Rio_Barrier.md |
| Evasion | 16    | Player Root/variable/PC_variables/Rio/Rio_Evasion.md |
| Fire Armor | 2    | Player Root/variable/PC_variables/Rio/Rio_Fire Armor.md |
| General Armor | 0    | Player Root/variable/PC_variables/Rio/Rio_General Armor.md |
| Ice Armor | 1    | Player Root/variable/PC_variables/Rio/Rio_Ice Armor.md |
| Physical Armor | 0    | Player Root/variable/PC_variables/Rio/Rio_Physical Armor.md |
| Spirit Armor | 0    | Player Root/variable/PC_variables/Rio/Rio_Spirit Armor.md |
| general_armor | 0    | Player Root/variable/PC_variables/Rio/Rio_general_armor.md |


**Consumable Resources**


| Resource Name            | Current/Max | Source File                                       |
| ------------------------ | ----------- | ------------------------------------------------- |
| Firebending slot         | 2/3         | Player Root/PCs/Rio/Rio Character Sheet.md        |
| Waterbottle charge       | 4           | Player Root/PCs/Rio/Rio Character Sheet.md        |


### Sora


**Vitality**


| Key           | Value | Source File                            |
| current_hp | 32    | Player Root/variable/PC_variables/Sora/Sora_current_hp.md |
| max_hp | 32    | Player Root/variable/PC_variables/Sora/Sora_max_hp.md |
| max_hp | 32    | Player Root/PCs/Sora/Sora Character Sheet.md |
| current_hp | 32    | Player Root/PCs/Sora/Sora Character Sheet.md |
| ready | no    | Player Root/PCs/Sora/Sora Character Sheet.md |


**Defensive**


| Key           | Value | Source File                            |
| Barrier | 0    | Player Root/variable/PC_variables/Sora/Sora_Barrier.md |
| Evasion | 12    | Player Root/variable/PC_variables/Sora/Sora_Evasion.md |
| Fire Armor | 0    | Player Root/variable/PC_variables/Sora/Sora_Fire Armor.md |
| General Armor | 0    | Player Root/variable/PC_variables/Sora/Sora_General Armor.md |
| Ice Armor | 3    | Player Root/variable/PC_variables/Sora/Sora_Ice Armor.md |
| Physical Armor | 0    | Player Root/variable/PC_variables/Sora/Sora_Physical Armor.md |
| Spirit Armor | 0    | Player Root/variable/PC_variables/Sora/Sora_Spirit Armor.md |
| general_armor | 0    | Player Root/variable/PC_variables/Sora/Sora_general_armor.md |


**Consumable Resources**


| Resource Name            | Current/Max | Source File                                       |
| ------------------------ | ----------- | ------------------------------------------------- |
| Firebending slot         | 9/9         | Player Root/PCs/Sora/Sora Character Sheet.md      |


### Tai


**Vitality**


| Key           | Value | Source File                            |
| current_hp | 53    | Player Root/variable/PC_variables/Tai/Tai_current_hp.md |
| max_hp | 60    | Player Root/variable/PC_variables/Tai/Tai_max_hp.md |
| max_hp | 60    | Player Root/PCs/Tai/Tai character sheet.md |
| current_hp | 53    | Player Root/PCs/Tai/Tai character sheet.md |
| ready | no    | Player Root/PCs/Tai/Tai character sheet.md |


**Defensive**


| Key           | Value | Source File                            |
| Barrier | 0    | Player Root/variable/PC_variables/Tai/Tai_Barrier.md |
| Evasion | 18    | Player Root/variable/PC_variables/Tai/Tai_Evasion.md |
| Fire Armor | 3    | Player Root/variable/PC_variables/Tai/Tai_Fire Armor.md |
| General Armor | 0    | Player Root/variable/PC_variables/Tai/Tai_General Armor.md |
| Ice Armor | 0    | Player Root/variable/PC_variables/Tai/Tai_Ice Armor.md |
| Physical Armor | 0    | Player Root/variable/PC_variables/Tai/Tai_Physical Armor.md |
| Spirit Armor | 0    | Player Root/variable/PC_variables/Tai/Tai_Spirit Armor.md |
| general_armor | 0    | Player Root/variable/PC_variables/Tai/Tai_general_armor.md |


**Consumable Resources**


| Resource Name            | Current/Max | Source File                                       |
| ------------------------ | ----------- | ------------------------------------------------- |
| Waterbottle charge       | 5/6         | Player Root/PCs/Tai/Tai character sheet.md        |


### Tapioca


**Vitality**


| Key           | Value | Source File                            |
| current_hp | 25    | Player Root/variable/PC_variables/Tapioca/Tapioca_current_hp.md |
| max_hp | 36    | Player Root/variable/PC_variables/Tapioca/Tapioca_max_hp.md |
| max_hp | 36    | Player Root/PCs/Tapioca/Tapioca Character Sheet.md |
| current_hp | 25    | Player Root/PCs/Tapioca/Tapioca Character Sheet.md |
| ready | no    | Player Root/PCs/Tapioca/Tapioca Character Sheet.md |


**Defensive**


| Key           | Value | Source File                            |
| Barrier | 0    | Player Root/variable/PC_variables/Tapioca/Tapioca_Barrier.md |
| Evasion | 19    | Player Root/variable/PC_variables/Tapioca/Tapioca_Evasion.md |
| Fire Armor | 0    | Player Root/variable/PC_variables/Tapioca/Tapioca_Fire Armor.md |
| General Armor | 1    | Player Root/variable/PC_variables/Tapioca/Tapioca_General Armor.md |
| Ice Armor | 0    | Player Root/variable/PC_variables/Tapioca/Tapioca_Ice Armor.md |
| Physical Armor | 0    | Player Root/variable/PC_variables/Tapioca/Tapioca_Physical Armor.md |
| Spirit Armor | 0    | Player Root/variable/PC_variables/Tapioca/Tapioca_Spirit Armor.md |
| general_armor | 1    | Player Root/variable/PC_variables/Tapioca/Tapioca_general_armor.md |


**Consumable Resources**


| Resource Name            | Current/Max | Source File                                       |
| ------------------------ | ----------- | ------------------------------------------------- |
| Airbending slot          | 2/6         | Player Root/PCs/Tapioca/Tapioca Character Sheet.m |
| Danger Sense Reactions   | 2/2         | Player Root/PCs/Tapioca/Tapioca Character Sheet.m |
| Earthbending slot        | 3/3         | Player Root/PCs/Tapioca/Tapioca Character Sheet.m |


### Tiebeediy


**Vitality**


| Key           | Value | Source File                            |
| current_hp | 32    | Player Root/variable/PC_variables/Tiebeediy/Tiebeediy_current_hp.md |
| max_hp | 32    | Player Root/variable/PC_variables/Tiebeediy/Tiebeediy_max_hp.md |
| max_hp | 32    | Player Root/PCs/Tiebeediy/Tiebeediy character sheet.md |
| current_hp | 32    | Player Root/PCs/Tiebeediy/Tiebeediy character sheet.md |


**Defensive**


| Key           | Value | Source File                            |
| Barrier | 0    | Player Root/variable/PC_variables/Tiebeediy/Tiebeediy_Barrier.md |
| Evasion | 13    | Player Root/variable/PC_variables/Tiebeediy/Tiebeediy_Evasion.md |
| Fire Armor | 0    | Player Root/variable/PC_variables/Tiebeediy/Tiebeediy_Fire Armor.md |
| General Armor | 1    | Player Root/variable/PC_variables/Tiebeediy/Tiebeediy_General Armor.md |
| Ice Armor | 0    | Player Root/variable/PC_variables/Tiebeediy/Tiebeediy_Ice Armor.md |
| Physical Armor | 0    | Player Root/variable/PC_variables/Tiebeediy/Tiebeediy_Physical Armor.md |
| Spirit Armor | 2    | Player Root/variable/PC_variables/Tiebeediy/Tiebeediy_Spirit Armor.md |


**Consumable Resources**


| Resource Name            | Current/Max | Source File                                       |
| ------------------------ | ----------- | ------------------------------------------------- |
| Earthbending slot        | 3/3         | Player Root/PCs/Tiebeediy/Tiebeediy character she |
| Spiritbending slot       | 2/2         | Player Root/PCs/Tiebeediy/Tiebeediy character she |


### X_Testchar


**Vitality**


| Key           | Value | Source File                            |
| current_hp | 2600    | Player Root/variable/PC_variables/X_Testchar/X_Testchar_current_hp.md |
| max_hp | 2600    | Player Root/variable/PC_variables/X_Testchar/X_Testchar_max_hp.md |
| max_hp | 2600    | Player Root/PCs/X_Testchar/X_Testchar character sheet.md |
| current_hp | 2600    | Player Root/PCs/X_Testchar/X_Testchar character sheet.md |


**Defensive**


| Key           | Value | Source File                            |
| Barrier | 0    | Player Root/variable/PC_variables/X_Testchar/X_Testchar_Barrier.md |
| Evasion | 32    | Player Root/variable/PC_variables/X_Testchar/X_Testchar_Evasion.md |
| Fire Armor | 20    | Player Root/variable/PC_variables/X_Testchar/X_Testchar_Fire Armor.md |
| General Armor | 20    | Player Root/variable/PC_variables/X_Testchar/X_Testchar_General Armor.md |
| Ice Armor | 20    | Player Root/variable/PC_variables/X_Testchar/X_Testchar_Ice Armor.md |
| Physical Armor | 0    | Player Root/variable/PC_variables/X_Testchar/X_Testchar_Physical Armor.md |
| Spirit Armor | 20    | Player Root/variable/PC_variables/X_Testchar/X_Testchar_Spirit Armor.md |


**Consumable Resources**


| Resource Name            | Current/Max | Source File                                       |
| ------------------------ | ----------- | ------------------------------------------------- |
| Airbending slot          | 60/60       | Player Root/PCs/X_Testchar/X_Testchar character s |
| Danger Sense Reactions   | 20/20       | Player Root/PCs/X_Testchar/X_Testchar character s |
| Firebending slot         | 60/60       | Player Root/PCs/X_Testchar/X_Testchar character s |
| Earthbending slot        | 60/60       | Player Root/PCs/X_Testchar/X_Testchar character s |
| Spiritbending slot       | 20/20       | Player Root/PCs/X_Testchar/X_Testchar character s |
| Waterbottle charge       | 40          | Player Root/PCs/X_Testchar/X_Testchar character s |


_Last generated by Mycelium/scripts/Python/generate_stat_overview.py_
