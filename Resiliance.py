...
def collect_variable_files(root: pathlib.Path) -> Tuple[Dict[str, List[pathlib.Path]], Dict[pathlib.Path, Optional[Dict[str, str]]]]:
    """Return mapping basename->list(paths) and per-path parsed table (or None)."""
...
def main(argv=None):
    ...
    var_map, parsed_tables = collect_variable_files(ROOT)
